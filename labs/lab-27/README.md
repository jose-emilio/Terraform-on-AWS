# Laboratorio 27 — Gobernanza, Documentación y Publicación "Lean"
...
```bash
cd labs/lab-27/aws/modules/secure-bucket
...
`pre-commit install` engancha los hooks al repositorio Git **donde se encuentre el `cd`**. Si lo ejecutas dentro de `labs/lab-27/aws`, Git localiza el repo padre (el del curso) y los hooks dispararían en cada commit a cualquier lab — no es lo que queremos.
...
```bash
cd ~/terraform-on-aws/labs/lab-27/aws/consumer
