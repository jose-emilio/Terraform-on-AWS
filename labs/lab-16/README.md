# Laboratorio 16 — Blindaje del Pipeline DevSecOps
...
# Validar código propio (p.ej. lab-14 que sí tiene S3)
conftest test *.tf \
  --policy ../policies/ \
  --parser hcl2 \
  --all-namespaces
...
# Ejemplo: arn:aws:iam::510547572113:role/lab16-github-actions
```
...
=== Identidad y nombre de sesión ===
{
    ...
    "Arn": "arn:aws:sts::510547572113:assumed-role/lab16-github-actions/GitHubActions-OIDC-Test"
}
```
...
| `assumed-role/lab16-github-actions/GitHubActions-OIDC-Test` en el ARN | El rol correcto fue asumido y la sesión lleva el nombre definido en `role-session-name` — útil para auditar en CloudTrail |
...
aws iam get-role \
  --role-name lab16-github-actions \
...
<tu-repo>/
├── .github/workflows/
│   └── terraform-ci.yml      ← el pipeline (de lab-16/pipeline/)
...
cp $LAB_REPO/labs/lab-16/pipeline/terraform-ci.yml .github/workflows/terraform-ci.yml

# 2. Copiar las políticas OPA (puedes empezar con las del lab y añadir más)
mkdir -p policies
cp $LAB_REPO/labs/lab-16/policies/*.rego policies/

# 3. Copiar el ejemplo Terraform mínimo de partida
mkdir -p terraform
cp $LAB_REPO/labs/lab-16/pipeline/terraform/* terraform/

git add .github/workflows/terraform-ci.yml policies/ terraform/
git commit -m "Activar pipeline DevSecOps de lab-16"
git push
```
...
Crea `labs/lab-16/policies/sg_no_public_ingress.rego` con el paquete, el
...
```bash
conftest test labs/lab-16/policies/fixtures/bad_sg.tf \
  --policy labs/lab-16/policies/ \
  --parser hcl2 \
  --all-namespaces
```
...
# Esperado: ["lab16-terraform-permissions"] (o el nombre que le hayas dado)

aws iam get-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name lab16-terraform-permissions \
...
data "aws_iam_policy_document" "terraform_permissions" {
  ...
  # IAM read-only para el refresh del estado del propio lab-16
  statement {
    sid    = "IAMReadOnly"
    effect = "Allow"
    actions = [
