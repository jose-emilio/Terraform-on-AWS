# Laboratorio 21 — Hub-and-Spoke con Transit Gateway y RAM
...
resource "aws_ec2_transit_gateway" "main" {
  description                     = "TGW central - Hub-and-Spoke lab21"
...
# tgw_flow_log_group          = "/tgw/lab21/flow-logs"
...
aws ec2 describe-transit-gateways \
  --filters Name=tag:Project,Values=lab21 \
...
aws ec2 describe-nat-gateways \
  --filter Name=tag:Project,Values=lab21 \
...
aws ram get-resource-shares \
  --resource-owner SELF \
  --tag-filters tagKey=Project,tagValues=lab21 \
...
**Cómo abrir la consola:** AWS Console → CloudWatch → Logs Insights → seleccionar log group `/tgw/lab21/flow-logs` → pegar la consulta de abajo → *Run query*.
...
Abre AWS Console → CloudWatch → Logs Insights → log group `/tgw/lab21/flow-logs` → pega esta consulta y *Run query*:
