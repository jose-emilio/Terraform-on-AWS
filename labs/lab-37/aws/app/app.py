...
PROJECT      = os.environ.get("PROJECT", "lab37")
...
def cache_flush_all(r):
    """Elimina todas las claves del namespace lab37:products:*."""
    if not r:
        return 0
    try:
        keys = r.keys("lab37:products:*")
        if keys:
            r.delete(*keys)
            return len(keys)
    except Exception:
...
def index():
    ...
    cache_key = f"lab37:products:{filter_cat or 'all'}:{filter_status or 'all'}"
...
def add_product():
    ...
    cache_invalidate(r, "lab37:products:all:all", f"lab37:products:{category}:all",
                     f"lab37:products:all:{status}", f"lab37:products:{category}:{status}")
...
def update_product():
    ...
    cache_invalidate(
        r,
        "lab37:products:all:all",
        f"lab37:products:{category}:all",
        f"lab37:products:all:{status}",
        f"lab37:products:{category}:{status}",
    )
